...
const controller = {
    ...
    getById: async (req, res) => {
        try {
            ...
            const fetchPayment = await paymentDb.findByPk(paymentId);
            
            if (!fetchPayment) {
                return res.status(404).json('Payment not found');
            }

            res.status(200).json(fetchPayment);
        } catch (err) {
            console.log(err);
    ...
    update: async (req, res) => {
        try {
            ...
            const payment = await paymentModel.findOne({ where: { UUID: uuid } });
            
            if (!payment) {
                return res.status(404).json('This payment link is invalid or expired.');
            }

            const newPayer = await payerModel.create({
                username,
                phone,
                email
            });
            ...
            const count = await paymentHistoryModel.count({ where: { idPay: payment.id } });
